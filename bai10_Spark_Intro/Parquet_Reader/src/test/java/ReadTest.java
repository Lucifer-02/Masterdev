...
public class ReadTest {
    @Test
    public void AccordingOfficial() throws AnalysisException {
...
//                .config("spark.master", "local")
        ...
        Dataset<Row> parquetFileDF = spark.read().parquet("data/small.parquet");
        ...
        device_model_num_user.repartition(1).write().mode("overwrite").option("compression", "snappy").format("parquet").save("output/device_model_num_user");
        device_model_list_user.repartition(1).write().mode("overwrite").format("orc").save("output/device_model_list_user");
        action_by_button_id.repartition(1).write().mode("overwrite").format("parquet").save("output/action_by_button_id");
    }
}
